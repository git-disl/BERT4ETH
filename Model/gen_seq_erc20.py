...
def load_data(f_in, f_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans):
    eoa2seq_out = {}
    error_trans = []
    while True:
        ...
        if trans_hash in erc20_trans:
            eoa_list = trans_to_log_eoa_list[trans_hash]
            erc20_num = len(eoa_list)
            if erc20_num > 5:
                eoa_list = list(np.random.permutation(eoa_list)[:5])
                erc20_num = 5
            to_and_eoa_address = ",".join([to_address] + eoa_list)

            try:
                eoa2seq_out[from_address].append(
                    [to_and_eoa_address, block_number, block_timestamp, value, "OUT", 1, "ERC20",
                     erc20_num])  # in/out, cnt
            except:
                eoa2seq_out[from_address] = [
                    [to_and_eoa_address, block_number, block_timestamp, value, "OUT", 1, "ERC20",
                     erc20_num]]  # in/out, cnt
            ...
            try:
                eoa2seq_out[from_address].append(
                    [to_address, block_number, block_timestamp, value, "OUT", 1, "TRANS", 0])  # in/out, cnt
            except:
                eoa2seq_out[from_address] = [
                    [to_address, block_number, block_timestamp, value, "OUT", 1, "TRANS", 0]]  # in/out, cnt
    ...
    while True:
        ...
        try:
            eoa2seq_in[to_address].append(
                [from_address, block_number, block_timestamp, value, "IN", 1, "TRANS", 0])  # not process trans
        except:
            eoa2seq_in[to_address] = [
                [from_address, block_number, block_timestamp, value, "IN", 1, "TRANS", 0]]  # in/out, cnt
...
def main():
    ...
    with open(os.path.join(FLAGS.data_dir, "exp_trans_to_erc20_eoa.pkl"), "rb") as f:
        trans_to_log_eoa_list = pkl.load(f)

    erc20_trans = set(list(trans_to_log_eoa_list.keys()))

    exp_erc20_trans = open(os.path.join(FLAGS.data_dir, "exp_erc20_trans.csv"), "r")
    trans_files = [exp_erc20_trans]
    hash_to_erc20_trans = {}
    ...
    eoa2seq_in, eoa2seq_out = load_data(f_in, f_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans)
    ...
    if FLAGS.phisher:
        ...
        phisher_eoa2seq_in, phisher_eoa2seq_out = load_data(phisher_f_in, phisher_f_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans)
    ...
    if FLAGS.deanon:
        ...
        dean_eoa2seq_in, dean_eoa2seq_out = load_data(dean_f_in, dean_f_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans)
    ...
    if FLAGS.mev:
        ...
        mev_eoa2seq_in, mev_eoa2seq_out = load_data(mev_f_in, mev_f_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans)
    ...
    if FLAGS.tornado:
        ...
        tornado_eoa2seq_in, tornado_eoa2seq_out = load_data(tornado_in, tornado_out, erc20_trans, trans_to_log_eoa_list, hash_to_erc20_trans)
