...
flags.DEFINE_integer("max_eoa_num", 5, "")
flags.DEFINE_bool("use_gate", True, "Whether to use GATE for erc20 aggregation")
...
def input_fn(input_files,
    ...
    name_to_features = {
        ...
        "input_ids":
            tf.FixedLenFeature([FLAGS.max_seq_length], tf.string),
        "input_positions":
        ...
        "masked_lm_ids":
            tf.FixedLenFeature([MAX_PREDICTIONS_PER_SEQ], tf.string),
...
def model_fn(features, mode, bert_config, vocab, init_checkpoint, learning_rate,
    ...
    input_ids = features["input_ids"]  # string
    input_positions = features["input_positions"]
    input_mask = features["input_mask"]
    input_io_flags = features["input_io_flags"]
    input_trans_flags = features["input_trans_flags"]
    input_values = features["input_values"]
    input_counts = features["input_counts"]
    input_erc20_nums = features["input_erc20_nums"]
    masked_lm_positions = features["masked_lm_positions"]
    masked_lm_ids = features["masked_lm_ids"]
    masked_lm_erc20_nums = features["masked_lm_erc20_nums"]
    masked_lm_weights = features["masked_lm_weights"]

    is_training = (mode == tf.estimator.ModeKeys.TRAIN)

    # split the input_ids
    input_ids = tf.reshape(input_ids, shape=[FLAGS.batch_size * FLAGS.max_seq_length])

    input_ids_split_sparse = tf.string_split(input_ids, delimiter=",")
    input_ids_split = tf.sparse_to_dense(sparse_indices=input_ids_split_sparse.indices,
                                         output_shape=[FLAGS.batch_size * FLAGS.max_seq_length, FLAGS.max_eoa_num + 1],
                                         sparse_values=input_ids_split_sparse.values, default_value="0")
    input_ids = tf.reshape(input_ids_split, shape=[FLAGS.batch_size, FLAGS.max_seq_length, FLAGS.max_eoa_num + 1])

    # split the masked_lm_ids
    masked_lm_ids = tf.reshape(masked_lm_ids, shape=[FLAGS.batch_size * MAX_PREDICTIONS_PER_SEQ])
    masked_lm_ids_split_sparse = tf.string_split(masked_lm_ids, delimiter=",")
    masked_lm_ids_split = tf.sparse_to_dense(sparse_indices=masked_lm_ids_split_sparse.indices,
                                             output_shape=[FLAGS.batch_size * MAX_PREDICTIONS_PER_SEQ,
                                                           FLAGS.max_eoa_num + 1],
                                             sparse_values=masked_lm_ids_split_sparse.values, default_value="0")
    masked_lm_ids = tf.reshape(masked_lm_ids_split,
                               shape=[FLAGS.batch_size, MAX_PREDICTIONS_PER_SEQ, FLAGS.max_eoa_num + 1])

    # convert to numbers
    input_ids = tf.string_to_number(input_ids, out_type=tf.int32)
    masked_lm_ids = tf.string_to_number(masked_lm_ids, out_type=tf.int32)
...
def get_masked_lm_output_negative_sampling(bert_config, input_tensor, output_weights, positions,
                                           label_ids, erc20_nums, label_weights, vocab):
    """Get loss and log probs for the masked LM."""
    ...
    with tf.variable_scope("cls/predictions"):
        ...
        label_ids_embedding, label_eoa_embedding_mean = modeling.eoa_embedding_split(label_ids, output_weights, erc20_nums)
        pos_output_weights = modeling.eoa_embedding_agg(label_ids_embedding, label_eoa_embedding_mean, erc20_nums, FLAGS.use_gate)
